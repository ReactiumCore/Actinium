    ...
    class User extends Parse.User {
        ...
        get init() {
            return async () => {
                ...
                const adminUser = await this.save(params, options);
        ...
        get currentUser() {
            return async (options, toObject = false) => {
            ...
            return async (params, options) => {
                ...
                const cacheKey = [
                    ...
                    Buffer.from(
                        _.compact([
                            ...
                            search,
                            _.compact(
                                fields.map(fld => op.get(params, fld)),
                            ).join('_'),
                        ]).join('_'),
                    ).toString('base64'),
                ];
                ...
                if (indexBy) {
                    op.set(
                        response,
                        'results',
                        _.indexBy(response.results, indexBy),
                    );
                }
        ...
        get retrieve() {
            return async (params, options) => {
                ...
                const { results = [] } = await this.User.list(params, options);
        ...
        get save() {
            return async (params, options) => {
                ...
                const current = await this.User.currentUser(options);
                ...
                return this.User.retrieve({ objectId: user.objectId }, options);
            };
        }
        ...
        get trash() {
            return async (params, options) => {
                ...
                if (!objectId)
                    return new Error('objectId is a required parameter');
        ...
        get Meta() {
            return {
                delete: async (params, options) => {
                    ...
                    let user = await this.User.retrieve(
                        _.clone(params),
                        options,
                    );
                    if (!user) return;
                ...
                update: async (params, options) => {
                    let user = await this.retrieve(_.clone(params), options);
                    ...
                    user = await this.save(
                        { meta: newMeta, objectId: user.objectId },
                        options,
                    );
        ...
        get Pref() {
            return {
                ...
                update: async (params, options) => {
                    ...
                    user = await this.save(
                        { pref: newPref, objectId: user.objectId },
                        options,
                    );
 ...
 * @apiGroup Actinium
 * @apiName User.trash
 * @apiDescription Send a single `Actinium.User` object to the recycle bin.
 ...
 * @apiParam (hooks) {Hook} user-before-delete Triggered before the `Actinium.User` object is deleted.

```
Arguments: req:Object:Actinium.User
```
 * @apiParam (hooks) {Hook} user-after-delete Triggered after the `Actinium.User` object is deleted.
