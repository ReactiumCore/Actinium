...
const SDK = Actinium => {
    ...
    Roles.can = (...args) => Actinium.Capability.Role.can(...args);

    Roles.capabilities = (...args) => Actinium.Capability.Role.get(...args);
    ...
    Roles.create = (roleObj = {}, options = { useMasterKey }) => {
        ...
        return ActionSequence({
            actions: {
                ...
                hook: () =>
                    Actinium.Hook.run('role-created', role, Roles.get()),
            },
        });
    };
    ...
    Roles.remove = (role, options = { useMasterKey }) =>
        ActionSequence({
            actions: {
                remove: () =>
                    Actinium.Cloud.run('role-remove', { role }, options),
                hook: () =>
                    Actinium.Hook.run('role-removed', role, Roles.get()),
            },
        });
    ...
    Roles.init = () =>
        Parse.Object.saveAll(
            ...
            roles = roles.map(role => {
                ...
                const roleData =
                    _.findWhere(Actinium.options.ROLES, { name }) || {};
